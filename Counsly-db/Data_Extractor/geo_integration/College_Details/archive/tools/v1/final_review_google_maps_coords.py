...
from validate_google_maps_coords import INDIA_BOUNDS, NAME_FIELDS, PLACE_PATTERNS, TAMIL_NADU_BOUNDS, distance_meters, extract_coords, within_bounds
...
class MapsSecondPassReviewer:
    ...
    def _inspect_page(self, page: Page, query: str, source: str, original_name: str) -> Candidate | None:
        ...
        score, accepted, reasons = score_candidate(original_name, title, body, place)
        return Candidate(
            query=query,
            source=source,
            title=title,
...
def score_candidate(original_name: str, candidate_title: str, body_text: str, coords: tuple[float, float] | None) -> tuple[float, bool, list[str]]:
