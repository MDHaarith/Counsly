...
export function RecommendationsClient({ initialData }: { initialData: RecommendationsPayload | null }) {
  ...
  return (
    <div className="space-y-4">
      {data.restriction === "ineligible" && (
        <Card>
          <h2 className="font-serif text-lg font-medium">Recommendations locked</h2>
          <p className="mt-1 text-sm leading-relaxed text-olive-gray">
            Your entered cutoff is below the guidance threshold, so Counsly will not show recommendation claims for this cycle.
          </p>
        </Card>
      )}
      {data.restriction === "data_not_ready" && (
        <Card>
          <h2 className="font-serif text-lg font-medium">Data not ready</h2>
          <p className="mt-1 text-sm leading-relaxed text-olive-gray">
            Verified cutoff data must be seeded before Counsly can recommend colleges.
          </p>
        </Card>
      )}
      {data.restriction === "plan_limit" && (
        <Card variant="featured">
          <h2 className="font-serif text-lg font-medium">
            Showing {data.returned} of {data.total}
          </h2>
          <p className="mt-1 text-sm text-olive-gray">Unlock all recommendations for this season.</p>
          <Link href="/subscribe?from=recommendations">
            <Button className="mt-4">Unlock Full Access</Button>
          </Link>
        </Card>
      )}
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-olive-gray">
                  {item.branch_name} · {item.district ?? "District pending"}
                </p>
              </div>
