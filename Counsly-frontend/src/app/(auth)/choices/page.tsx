...
export default function ChoicesPage() {
  const [data, setData] = useState<ChoicesPayload | null>(null);
  ...
  useEffect(() => {
    apiClient<ChoicesPayload>("/api/choices")
      .then((payload) => {
        setData(payload);
        setError(null);
      })
      .catch((err) => setError(err instanceof Error ? err.message : "Could not load choices."));
  }, []);
  ...
  async function updateChoice(choice: ChoiceItem, patch: { notes?: string | null; manual_category?: SafetyLabel | null }) {
    setMoving(choice.id);
    try {
      setData(await apiClient<ChoicesPayload>(`/api/choices/${choice.id}`, {
        method: "PATCH",
        body: JSON.stringify({
  ...
  return (
    <div className="space-y-4 p-5">
      <div>
        ...
        {data && <p className="mt-2 text-sm text-olive-gray">{data.items.length}/{data.limit} active rows · tap a priority number to move.</p>}
      </div>
      {data && (
          ...
          <Card key={choice.id}>
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-sm text-olive-gray">{choice.branch_name ?? choice.branch_code} · {choice.district ?? "District pending"}</p>
                <div className="mt-2 flex gap-2">{choice.manual_category && <Badge variant={choice.manual_category}>manual {choice.manual_category}</Badge>}{choice.system_category && <Badge variant={choice.system_category}>{choice.system_category}</Badge>}</div>
              </div>
            </div>
                ...
                {(["safe", "moderate", "ambitious"] as SafetyLabel[]).map((category) => (
                  <button
                    ...
                    className={[
                      ...
                      choice.manual_category === category ? "border-anthracite bg-warm-sand text-anthracite" : "border-border-cream bg-ivory text-olive-gray",
                    ].join(" ")}
                  >
                    {category}
                  </button>
                ))}
                <Button variant="ghost" className="w-auto px-3 text-error-crimson" onClick={() => deleteChoice(choice)}>Remove</Button>
              </div>
            </div>
      ...
      <Card>
        ...
        <Button className="mt-3" onClick={addChoice} disabled={saving}>{saving ? "Adding" : "Add to list"}</Button>
      </Card>
    </div>
  );
}
